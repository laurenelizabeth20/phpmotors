...
function getAdditionalImages($invId){
    $db = phpmotorsConnect();
    $sql = 'SELECT imgPath FROM images WHERE invId = :invId AND imgPrimary = 0 AND imgName LIKE "%-tn.jpg"';
    $stmt = $db->prepare($sql);
    $stmt->bindValue(':invId', $invId, PDO::PARAM_STR);
    $stmt->execute();
    $additionalImages = $stmt->fetch(PDO::FETCH_ASSOC);
    $stmt->closeCursor();
